...
async def test_profiles(create_all_controllers, create_image):
        ...
        fake_image = Image.open(create_image)
        user = UsersDTO(id=1, **full_user_data)
        user_post = UsersPostDTO(**full_user_data)

        assert await profiles.create_profile(1, user_post)
        assert await profiles.add_profile_picture(fake_image, 1)
        assert await profiles.set_profile_picture(1, 1)
        assert await profiles.add_image(fake_image, 1)
        ...
        assert await profiles.delete_image(1, 1)
        assert await profiles.delete_profile_picture(1)
